 ...
 * @updated     2026-01-17T02:49:56+01:00
 * @author      Akki Scholze
 * @changelog
 *   0.4.2 - 2026-01-17 - Fix TypeScript errors: bg.content→bg.main, text.primary→text.active, text.secondary→text.inactive
...
export function Dashboard() {
  ...
  const contentStyle: CSSProperties = {
    backgroundColor: appConfig.theme.colors.bg?.main || '#1a1a1a',
    padding: areaPadding,
    minHeight: '400px'
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      <ContentArea style={contentStyle}>
        <p style={{ color: appConfig.theme.colors.text.active }}>Content Area - Page wird hier geladen</p>
      </ContentArea>
      <FooterArea style={footerStyle} columns={3}>
        <div style={{ color: appConfig.theme.colors.text.inactive }}>Footer Left</div>
        <div style={{ color: appConfig.theme.colors.text.inactive }}>Footer Center</div>
        <div style={{ color: appConfig.theme.colors.text.inactive }}>Footer Right</div>
      </FooterArea>
    </div>
  );
}
